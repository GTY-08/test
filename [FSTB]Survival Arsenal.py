...
wave_font = pygame.font.Font('NeoDunggeunmoPro-Regular.ttf', 85)
menu_font = pygame.font.Font('NeoDunggeunmoPro-Regular.ttf', 100)
title_font = pygame.font.Font('NeoDunggeunmoPro-Regular.ttf', 130)

# 화면 크기 설정
screen_width = 1920
screen_height = 1080
screen = pygame.display.set_mode((screen_width, screen_height), pygame.DOUBLEBUF)
pygame.display.set_caption('Survival Arsenal')
...
explosion_sound = pygame.mixer.Sound("펑.mp3")
...
bullet_image = pygame.transform.scale(bullet_image, (12, 26))
...
#총 이미지
handgun_image = pygame.image.load('handgun.png')
handgun_image = pygame.transform.scale(handgun_image, (25, 110))

#조준점 이미지
pointer_image = pygame.image.load('pointer.png')
pointer_image = pygame.transform.scale(pointer_image, (120, 120))
big_pointer_image = pygame.image.load('big pointer.png')
...
# 플레이어 설정
player_image = pygame.image.load('player.png')  # 업로드한 이미지 파일
...
dark = pygame.transform.scale(dark, (1920, 1080))  
...
# 음악 파일 경로 설정
menu_music_path = 'main.mp3'
play_music_path = 'battle.mp3'
      
# 음악 전환 함수
def switch_music(is_menu):
    music_path = menu_music_path if is_menu else play_music_path
    pygame.mixer.music.load(music_path)
    pygame.mixer.music.play(-1)

# 초기 메뉴 음악 재생
switch_music(is_menu=True)

def player_starts_game():
    return False

def player_exits_to_menu():
    return False      
# 함수: 화면 진동 시작
...
weapon_special_skills = {
    "bow": {"cooldown": 3000, "skill": "multi_arrow"},
    "handgun": {"cooldown": 8000, "skill": "rapid_fire"},
    "magic": {"cooldown": 6000, "skill": "fire_blast"},
}
...
def use_special_skill(weapon_type, weapon_x, weapon_y, angle):
    global last_skill_use_time, is_rapid_firing, rapid_fire_end_time, current_ammo, player_y, player_x
    current_time = pygame.time.get_ticks()
    ...
    if current_time - last_skill_use_time[weapon_type] >= skill_data["cooldown"]:
        ...
        if skill_data["skill"] == "multi_arrow":
            ...
            # 넓은 범위 불 마법 (마법)
            projectiles.append(BigFireProjectile(player_x, player_y, pointer_x, pointer_y))
...
class FireProjectile:
    ...
    def explode(self, enemies):
        ...
        explosion_effect_color = (255, 0, 0)  # 폭발 이펙트 색상 
...
class BigFireProjectile:
    def __init__(self, x, y, target_x, target_y):
        self.angle = math.degrees(math.atan2(target_y - y, target_x - x))
        self.x = x 
        self.y = y 
        self.speed = 11  # 원하는 속도 설정
        self.radius = 60
        self.image = pygame.transform.scale(fire_image, (200, 110))  # 불 크기 조정
        self.exploded = False
        self.explosion_radius = 240  # 폭발 반경
        self.explosion_damage = 25  # 폭발에 의한 피해량
        self.to_remove = False  # 삭제 플래그
    ...
    def explode(self, enemies):
        ...
        for _ in range(3) :
            ...
            pygame.draw.circle(screen, explosion_effect_color, (int(self.x + random.randint(-50,50)) , int(self.y+ random.randint(-50,50))) , self.explosion_radius)  # 폭발 반경을 표시
...
def draw_menu():
    ...
    start_button_rect = pygame.Rect(screen_width // 2 - 100, 400, 180, 80)
    exit_button_rect = pygame.Rect(screen_width // 2 - 100, 700, 180, 80)
    while True:
        screen.fill(GROUND_COLOR)  # 배경 색상

        # 제목 텍스트
        title_text = title_font.render("Survival Arsenal", True, (255, 255, 255))
        screen.blit(title_text, (screen_width // 2 - title_text.get_width() // 2, 80))
        
        # Start 버튼 그리기
        pygame.draw.rect(screen, GROUND_COLOR, start_button_rect)  # 버튼 배경 색상
        start_text = menu_font.render("Play", True, (255, 255, 255))
        screen.blit(start_text, (screen_width // 2 - start_text.get_width() // 2, 400))
        
        # 종료 버튼 그리기
        pygame.draw.rect(screen, GROUND_COLOR, exit_button_rect)  # 버튼 배경 색상
        start_text = menu_font.render("exit", True, (255, 255, 255))
        screen.blit(start_text, (screen_width // 2 - start_text.get_width() // 2, 700))
        
        
        if dark_alpha > 0:
            dark_alpha -= 4  # 투명도 감소 속도 조절 (값을 조정하여 속도를 조절)
            dark.set_alpha(dark_alpha)  # 이미지에 새로운 투명도 설정
            screen.blit(dark, (0, 0))  # 화면에 이미지 그리기
...
while True :
    ...
    running = True
    while running:
        selected_weapon = draw_menu()  
        player_hp = player_Maxhp 
        switch_music(is_menu=False) 
        running = False
    ...
    while running:
        ...
        if is_rapid_firing and current_ammo > 0:
            if current_time < rapid_fire_end_time:
                projectiles.append(Projectile(weapon_x , weapon_y , angle + random.randint(-3, 3)))
    ...
    # 투사체 업데이트
        for projectile in projectiles[:]:
            ...
            for enemy in enemies[:]:
                ...
                if dist < enemy.radius + projectile.radius :
                    ...
                    if isinstance(projectile, FireProjectile) or isinstance(projectile, BigFireProjectile):
        ...
        if enemy_spawn_warning_time is None and wave_current_time - last_enemy_spawn_time > enemy_spawn_delay and wave_current_time > 2500:
            ...
            enemy_spawn_warning_time = wave_current_time 
        ...
        if enemy_spawn_warning_time and wave_current_time - enemy_spawn_warning_time >= enemy_warning_time and wave_current_time > 4000:
            ...
            if wave_current_time < 20000:
                ...
                new_enemy = spawn_random_enemy(random.choice([MutantEnemy, StoneEnemy, Titan, FlashEnemy, SuicideEnemy, InvisibleEnemy]))
                enemy_warning_time = 1000
                enemy_spawn_delay = 1000  
                wave_duration = 100000000
        ...
        if last_level_up_time and current_time - last_level_up_time < level_up_display_time:
            ...
            screen.blit(level_up_text, (player_x - level_up_text.get_width() // 2, player_y - 155))
              
        level_text = font.render(f"Lv. {player_level}", True, (255, 255, 255))  # 플레이어 레벨 텍스트 생성
        screen.blit(level_text, (player_x - player_hp_bar_width // 2 + (player_hp_bar_width - level_text.get_width()) // 2, player_y - 125))  # 체력바 왼쪽에 표시
        
        pygame.draw.rect(screen, (0, 0, 0), (player_x - player_hp_bar_width // 2 + shake_offset_x, player_y - 85 + shake_offset_y, player_hp_bar_width, player_hp_bar_height))  # 플레이어 HP 바 배경
        pygame.draw.rect(screen, (255, 0, 0), (player_x - player_hp_bar_width // 2 + shake_offset_x, player_y - 85 + shake_offset_y, player_hp_bar_width * (player_hp / player_Maxhp), player_hp_bar_height))  # 플레이어 체력
        ...
        # 경험치 바 배경
        pygame.draw.rect(screen, (0, 0, 0), (player_x - experience_bar_width // 2+ shake_offset_x, player_y - 114 + player_hp_bar_height+ shake_offset_y , experience_bar_width, experience_bar_height))

        # 현재 경험치 표시
        pygame.draw.rect(screen, (0, 255, 150), (player_x - experience_bar_width // 2+ shake_offset_x, player_y - 114 + player_hp_bar_height+ shake_offset_y , experience_bar_width * (player_experience / experience_to_level_up), experience_bar_height))
        
        # 스킬 쿨타임 바 설정
        skill_cooldown_bar_max_width = player_hp_bar_width  # 쿨타임 바 최대 너비
        skill_cooldown_bar_height = 8  # 쿨타임 바 높이
        
        # 쿨타임 바 그리기
        skill_data = weapon_special_skills[weapon_type]
        cooldown_time = skill_data["cooldown"]
        elapsed_time = current_time - last_skill_use_time[weapon_type]

        cooldown_ratio = min(elapsed_time / cooldown_time, 1)
        
        # 쿨타임 바 너비 계산
        cooldown_bar_width = skill_cooldown_bar_max_width * cooldown_ratio

        # 스킬 쿨타임 바 배경 그리기
        pygame.draw.rect(screen, (50, 50, 50), (player_x - skill_cooldown_bar_max_width // 2 + shake_offset_x, player_y - 60 + shake_offset_y, skill_cooldown_bar_max_width, skill_cooldown_bar_height))
        if int(cooldown_ratio) == 1 :
            # 스킬 쿨타임 진행 바 그리기
            pygame.draw.rect(screen, (20, 120, 255), (player_x - skill_cooldown_bar_max_width // 2 + shake_offset_x, player_y - 60 + shake_offset_y, cooldown_bar_width, skill_cooldown_bar_height))
        else:
            pygame.draw.rect(screen, (0, 0, 175), (player_x - skill_cooldown_bar_max_width // 2 + shake_offset_x, player_y - 60 + shake_offset_y, cooldown_bar_width, skill_cooldown_bar_height))
            

        for enemy in enemies:
            if not isinstance(enemy, InvisibleEnemy) or not enemy.invisible:
                enemy_hp_bar_width = player_hp_bar_width  
                enemy_hp_bar_height = 25
                pygame.draw.rect(screen, (0, 0, 0), (enemy.x - enemy_hp_bar_width // 2+ shake_offset_x, enemy.y - 85+ shake_offset_y, enemy_hp_bar_width, enemy_hp_bar_height))  # 적 HP 바 배경
                pygame.draw.rect(screen, (0, 0, 255), (enemy.x - enemy_hp_bar_width // 2+ shake_offset_x, enemy.y - 85+ shake_offset_y, enemy_hp_bar_width * (enemy.hp / enemy.Maxhp), enemy_hp_bar_height))  # 적 체력
        ...
        for cross in crosses[:]:
            ...
            # 십자가 화면에 블릿
            screen.blit(cross_surface, (cross_pos[0] - cross_size + shake_offset_x , cross_pos[1] - cross_size + shake_offset_y))
