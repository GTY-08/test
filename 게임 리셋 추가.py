...
# 폰트 설정
menu_font = pygame.font.Font('C:/Users/User/Desktop/startcoding/NeoDunggeunmoPro-Regular.ttf', 70)
...
invisible_enemy_image = pygame.transform.scale(invisible_enemy_image, (75, 75))  # 적 이미지 크기 조정

#적 이미지 지정
enemy_imagenumber = [enemy_image, fast_enemy_image, giant_enemy_image, invisible_enemy_image]
enemy_images = [enemy_image, fast_enemy_image, giant_enemy_image, invisible_enemy_image]
...
def reset_game():
    global player_x, player_y, player_hp, enemies, projectiles, current_ammo, total_magazines, is_reloading
    global player_experience, player_level, player_Maxhp, damage_up, last_enemy_spawn_time
...
class Enemy:
    def __init__(self, x, y, hp, speed, size, number, enemy_weight, damage):
        ...
        self.speed = speed *1.4 # 적의 기본 속도
        self.base_speed = speed * 1.4# base_speed 추가
...
class InvisibleEnemy(Enemy):
    def __init__(self, x, y):
        super().__init__(x, y, 200, 6, 37, 3, 1, 15)
        self.exp_reward = 70
        self.invisible = False
...
class StrongEnemy(Enemy):
    def __init__(self, x, y):
        super().__init__(x, y, 400, 4, 50, 2, 1.5, 15)  # 큰 적
        self.exp_reward = 50
...
class FastEnemy(Enemy):
    def __init__(self, x, y):
        super().__init__(x, y, 100, 8, 37, 1, 1, 20)  # 빠른 적
        self.exp_reward = 30
...
def spawn_random_enemy():
    ...
    enemy_type = random.choice([FastEnemy, StrongEnemy, NormalEnemy, InvisibleEnemy])
