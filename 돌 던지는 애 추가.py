...
clock = pygame.time.Clock()  # 프레임 속도 제어용 객체 생성
FPS = 30  # 목표 프레임 속도 설정
...
acceleration = 0.55  # 가속도
friction = 0.975  # 마찰(감속 비율)
projectile_speed = 50  # 투사체 속도
player_Maxhp = 100
player_hp = 100  # 플레이어 HP
...
#적 이미지 지정
enemy_imagenumber = [enemy_image, fast_enemy_image, giant_enemy_image, invisible_enemy_image,flash_enemy_image, stone_enemy_image,titan_image]
enemy_images = [enemy_image, fast_enemy_image, giant_enemy_image, invisible_enemy_image,flash_enemy_image, stone_enemy_image,titan_image]
...
def reset_game():
    ...
    global player_experience, player_level, player_Maxhp, damage_up, last_enemy_spawn_time, enemy_spawn_delay , enemy_warning_time, player_speed_x, player_speed_y
...
def add_experience(amount):
    global player_experience, player_level, experience_to_level_up, player_hp, player_Maxhp, weapon_damage, damage_up, last_level_up_time
...
class FastEnemy(Enemy):
    def __init__(self, x, y):
        super().__init__(x, y, 250, 9, 37, 1, 1, 50)  # 빠른 적
        self.exp_reward = 200
        
# 타이탄 클래스
class Titan(Enemy):
    def __init__(self, x, y):
        super().__init__(x, y, 1000, 3.5, 50, 6, 1.75, 60)  # 큰 적
        self.exp_reward = 225
...
class StoneEnemy(Enemy):  # Enemy 클래스를 상속받음
    def __init__(self, x, y):
        super().__init__(x, y, 400, 5, 37, 5, 1, 25)
        self.exp_reward = 160
        self.throw_delay = 2500  # 돌 던지기 딜레이 (초)
    ...
    def throw_stone(self, player_x, player_y):
        ...
        # 던지기 조건 확인 후 돌 생성
        if distance <= 900 and current_time - self.last_throw_time >= self.throw_delay:
            stone_speed = 30  # 돌의 속도 설정
...
class FlashEnemy(Enemy):
    ...
    def teleport_toward_player(self, player_x, player_y):
        current_time = pygame.time.get_ticks()
        if current_time - self.last_teleport_time >= random.randint(500,1000):  # 2초마다 실행
            # 플레이어 방향으로 각도 계산
...
class InvisibleEnemy(Enemy):
    def __init__(self, x, y):
        super().__init__(x, y, 350, 7, 37, 3, 1, 25)
        self.exp_reward = 90
        self.invisible = False
        self.last_toggle_time = time.time()
...
while True :
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if weapon_type == 'handgun' : 
                    ...
                    if current_time - last_shot_time > shoot_delay:
                        # 플레이어의 현재 각도에 따라 투사체 생성
                        projectiles.append(Arrow(weapon_x, weapon_y, angle))  # 무기 끝에서 발사
                        gunshot_sound.play()  # 총쏘는 소리 재생
                        last_shot_time = current_time  # 마지막 발사 시간을 현재 시간으로 갱신
    ...
    # 투사체 업데이트
        ...
        if enemy_spawn_warning_time and wave_current_time - enemy_spawn_warning_time >= enemy_warning_time and wave_current_time > 4000:
            ...
            if wave_current_time < 20000:
                ...
                wave = 11
                new_enemy = spawn_random_enemy(random.choice([StoneEnemy, Titan, InvisibleEnemy]))
                enemy_warning_time = 1500
                enemy_spawn_delay = 2000
    ...
    #     pygame.draw.rect(screen, WARNING_COLOR, (new_enemy_x - 15, new_enemy_y - 30, 50, 50))  # 적 생성 경고 사각형
        ...
        wave_text = menu_font.render(f"Wave {wave}", True, (255, 255, 255))  # 텍스트 생성
        screen.blit(wave_text, (850, 50))  # 텍스트 위치  
